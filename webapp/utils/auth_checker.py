"""
Authentication and Authorization Checker for SecOps Webapp

Validates gcert and admin_session status before allowing webapp access.
"""

import os
import subprocess
import logging
from datetime import datetime
from pathlib import Path
from typing import Tuple, Dict

from config import AUTH_DOMAIN


def check_admin_session() -> Tuple[bool, str]:
    """
    Check if user is in an admin_session by walking the process tree.

    admin_session doesn't export env vars to child processes and PS1 is
    shell-local, so heuristic checks fail. Instead we walk up the process
    tree looking for an 'admin_session' ancestor process.

    Returns:
        Tuple[bool, str]: (is_valid, message)
    """
    import re

    try:
        pid = os.getpid()
        for _ in range(20):  # safety limit
            result = subprocess.run(
                ['ps', '-o', 'ppid=,command=', '-p', str(pid)],
                capture_output=True,
                text=True,
                timeout=2
            )
            line = result.stdout.strip()
            if not line:
                break

            if 'admin_session' in line.lower():
                match = re.search(r"--reason[= ]*['\"]?([^'\"\\s]+)", line)
                if match:
                    return True, f"Active (Reason: {match.group(1)})"
                return True, "Active (detected in process tree)"

            parts = line.split(None, 1)
            if not parts:
                break
            try:
                parent_pid = int(parts[0])
            except ValueError:
                break
            if parent_pid <= 1 or parent_pid == pid:
                break
            pid = parent_pid

    except Exception:
        pass

    return False, "Not detected. Run: admin_session --reason='YOUR_TICKET_ID'"


def check_gcert() -> Tuple[bool, str]:
    """
    Check if gcert is valid and not expired.

    Uses the `gcertstatus` command which outputs lines like:
        LOAS2 expires in 16h 48m
        corp/normal expires in 19h 23m

    Falls back to file-based heuristic if gcertstatus is not available.

    Returns:
        Tuple[bool, str]: (is_valid, message)
    """
    # Try gcertstatus command first
    try:
        result = subprocess.run(
            ['gcertstatus'],
            capture_output=True,
            text=True,
            timeout=5
        )
        output = result.stdout.strip()
        if output:
            import re
            # Parse expiry lines like "LOAS2 expires in 16h 48m"
            expiry_entries = []
            for line in output.split('\n'):
                line = line.strip()
                if not line:
                    continue
                # Check for "expires in" pattern
                match = re.search(r'(.+?)\s+expires\s+in\s+(.+)', line)
                if match:
                    cert_type = match.group(1).strip()
                    time_remaining = match.group(2).strip()
                    expiry_entries.append((cert_type, time_remaining))
                # Check for "expired" or "not found" patterns
                elif 'expired' in line.lower() or 'not found' in line.lower() or 'no cert' in line.lower():
                    return False, f"gcert expired or not found. Run: gcert"

            if expiry_entries:
                summary = ", ".join(f"{t}: {r}" for t, r in expiry_entries)
                return True, f"Valid ({summary})"

            # If we got output but couldn't parse it, check return code
            if result.returncode == 0:
                return True, f"Valid (gcertstatus returned OK)"
            return False, f"gcert status unclear. Run: gcert"
    except FileNotFoundError:
        pass  # gcertstatus not available, fall back to file-based check
    except subprocess.TimeoutExpired:
        pass  # Timed out, fall back
    except Exception:
        pass  # Any other error, fall back

    # Fallback: file-based heuristic
    ssh_dir = Path.home() / '.ssh'

    cert_patterns = [
        '*google*cert*',
        '*certificateauthority*',
        '*.pub-cert',
        'id_*-cert.pub'
    ]

    cert_files = []
    if ssh_dir.exists():
        for pattern in cert_patterns:
            found = list(ssh_dir.glob(pattern))
            cert_files.extend(found)

    if not cert_files:
        return False, f"Unable to find gcert files in {ssh_dir}. Run: gcert"

    most_recent = max(cert_files, key=lambda p: p.stat().st_mtime)
    cert_age_seconds = datetime.now().timestamp() - most_recent.stat().st_mtime
    cert_age_hours = cert_age_seconds / 3600

    if cert_age_hours > 20:
        return False, f"Certificate may be expired (age: {cert_age_hours:.1f}h). Run: gcert"

    return True, f"Valid (age: {cert_age_hours:.1f}h, file: {most_recent.name})"


def check_gcloud_auth() -> Tuple[bool, str]:
    """
    Check if gcloud is authenticated with a corporate account.

    Runs ``gcloud auth list`` and checks for an active corporate account.

    Returns:
        Tuple[bool, str]: (is_valid, message)
    """
    try:
        result = subprocess.run(
            ['gcloud', 'auth', 'list', '--filter=status:ACTIVE', '--format=value(account)'],
            capture_output=True,
            text=True,
            timeout=10
        )
        if result.returncode == 0 and result.stdout.strip():
            account = result.stdout.strip().split('\n')[0]
            domain_suffix = f"@{AUTH_DOMAIN}"
            if domain_suffix in account:
                return True, f"Authenticated as {account}"
            elif '@' in account:
                return False, f"Authenticated as {account} — use your {domain_suffix} account"
            else:
                return False, f"Unexpected account format: {account}"
        return False, "No active gcloud account. Run: gcloud auth login"
    except FileNotFoundError:
        return False, "gcloud CLI not found"
    except subprocess.TimeoutExpired:
        return False, "gcloud command timed out"
    except Exception as e:
        return False, f"Error checking gcloud auth: {str(e)}"


def get_auth_status() -> Dict[str, any]:
    """
    Get comprehensive authentication status.

    Returns:
        Dict with keys: admin_session_valid, gcert_valid, gcloud_valid,
        admin_message, gcert_message, gcloud_message, all_valid
    """
    admin_valid, admin_msg = check_admin_session()
    gcert_valid, gcert_msg = check_gcert()
    gcloud_valid, gcloud_msg = check_gcloud_auth()

    return {
        'admin_session_valid': admin_valid,
        'gcert_valid': gcert_valid,
        'gcloud_valid': gcloud_valid,
        'admin_session_message': admin_msg,
        'gcert_message': gcert_msg,
        'gcloud_message': gcloud_msg,
        'all_valid': admin_valid and gcert_valid and gcloud_valid
    }


def validate_startup_requirements() -> bool:
    """
    Validate all startup requirements.

    Note: Changed to WARNING mode - returns True to allow startup even if checks fail.
    Displays warnings but does not block webapp from starting.

    Returns:
        bool: Always returns True (warnings only)
    """
    status = get_auth_status()

    logging.info("=" * 80)
    logging.info("AUTHENTICATION STATUS CHECK")
    logging.info("=" * 80)

    if not status['admin_session_valid']:
        logging.warning(f"⚠️  Admin Session: {status['admin_session_message']}")
        logging.warning("   If you're in admin_session, this detection may fail in subprocesses")
    else:
        logging.info(f"✓ Admin Session: {status['admin_session_message']}")

    if not status['gcert_valid']:
        logging.warning(f"⚠️  gcert: {status['gcert_message']}")
        logging.warning("   If your gcert is valid, check certificate file patterns")
    else:
        logging.info(f"✓ gcert: {status['gcert_message']}")

    if not status['gcloud_valid']:
        logging.warning(f"⚠️  gcloud: {status['gcloud_message']}")
        logging.warning("   Run: gcloud auth login")
    else:
        logging.info(f"✓ gcloud: {status['gcloud_message']}")

    if not status['all_valid']:
        logging.warning("=" * 80)
        logging.warning("⚠️  Some authentication checks could not be verified")
        logging.warning("   The webapp will start, but please verify manually:")
        logging.warning("   1. Ensure you're in admin_session")
        logging.warning("   2. Ensure gcert is valid")
        logging.warning(f"   3. Ensure gcloud is authenticated with @{AUTH_DOMAIN}")
        logging.warning("   4. Check the status banner in the webapp UI")
        logging.warning("=" * 80)
    else:
        logging.info("✓ All authentication checks passed")
        logging.info("=" * 80)

    # Always return True - allow startup with warnings
    return True
